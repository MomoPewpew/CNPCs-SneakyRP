		...
		if (args.length == 0) {
			...
			while (var8.hasNext()) {
				...
				this.sendMessage(sender,
						(String) entry.getKey() + ": " + ((CommandNoppesBase) entry.getValue()).getUsage(sender),
						new Object[0]);
			}
				...
				if (m == null) {
					...
					while (var6.hasNext()) {
						...
						sender.sendMessage(new TextComponentTranslation(
								(String) entry.getKey() + ": "
										+ ((CommandNoppesBase.SubCommand) ((Method) entry.getValue())
												.getAnnotation(CommandNoppesBase.SubCommand.class)).desc(),
								new Object[0]));
					}
				} else {
					this.sendMessage(sender,
							"------" + command.getName() + "." + args[1].toLowerCase() + " Command------",
							new Object[0]);
					CommandNoppesBase.SubCommand sc = (CommandNoppesBase.SubCommand) m
							.getAnnotation(CommandNoppesBase.SubCommand.class);
