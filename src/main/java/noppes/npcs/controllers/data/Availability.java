...
public class Availability implements ICompatibilty, IAvailability {
	...
	public boolean isAvailable(EntityPlayer player) {
		...
		if (!this.dialogAvailable(this.dialogId, this.dialogAvailable, player)) {
			...
			return false;
		} else if (!this.scoreboardAvailable(player, this.scoreboardObjective, this.scoreboardType,
				this.scoreboardValue)) {
			return false;
		} else if (!this.scoreboardAvailable(player, this.scoreboard2Objective, this.scoreboard2Type,
				this.scoreboard2Value)) {
			return false;
		} else {
