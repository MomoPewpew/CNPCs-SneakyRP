...
public class CustomGuiController {
	public static void openGui(PlayerWrapper player, CustomGuiWrapper gui) {
		((EntityPlayerMP) player.getMCEntity()).openGui(CustomNpcs.instance, EnumGuiType.CustomGui.ordinal(),
				player.getWorld().getMCWorld(), gui.getSlots().size(), 0, 0);
		((ContainerCustomGui) ((EntityPlayerMP) player.getMCEntity()).openContainer).setGui(gui,
				(EntityPlayer) player.getMCEntity());
