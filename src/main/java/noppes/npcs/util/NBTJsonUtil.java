...
public class NBTJsonUtil {
	...
	public static void FillCompound(NBTTagCompound compound, NBTJsonUtil.JsonFile json)
			throws NBTJsonUtil.JsonException {
				...
				if (s.isEmpty()) {
					...
					try {
						if (s.endsWith("d")) {
							...
							return (NBTBase) (s.contains(".") ? new NBTTagDouble(Double.parseDouble(s))
									: new NBTTagInt(Integer.parseInt(s)));
						}
	...
	private static NBTJsonUtil.JsonLine ReadTag(String name, NBTBase base, List list) {
		...
		if (base.getId() == 9) {
			...
			Object key;
			for (Iterator var10 = compound.getKeySet().iterator(); var10
					.hasNext(); line = ReadTag(key.toString(), compound.getTag(key.toString()), list)) {
				key = var10.next();
			}
