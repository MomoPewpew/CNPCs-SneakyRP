	...
	public ContainerNPCCompanion(EntityNPCInterface npc, EntityPlayer player) {
		...
		if (this.role.getTalentLevel(EnumCompanionTalent.ARMOR) > 0) {
			this.addSlotToContainer(
					new SlotCompanionArmor(this.role, npc.inventory, 0, 6, 8, EntityEquipmentSlot.HEAD));
			this.addSlotToContainer(
					new SlotCompanionArmor(this.role, npc.inventory, 1, 6, 26, EntityEquipmentSlot.CHEST));
			this.addSlotToContainer(
					new SlotCompanionArmor(this.role, npc.inventory, 2, 6, 44, EntityEquipmentSlot.LEGS));
			this.addSlotToContainer(
					new SlotCompanionArmor(this.role, npc.inventory, 3, 6, 62, EntityEquipmentSlot.FEET));
		}
