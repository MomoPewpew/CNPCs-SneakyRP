...
public class ScriptPlayerEventHandler {
	...
	public void invoke(RightClickItem event) {
		if (!event.getEntityPlayer().world.isRemote && event.getHand() == EnumHand.MAIN_HAND
			...
			if (event.getEntityPlayer().isCreative() && event.getEntityPlayer().isSneaking()
				...
				NoppesUtilServer.sendOpenGui(event.getEntityPlayer(), EnumGuiType.ScriptItem,
						(EntityNPCInterface) null);
			} else {
				PlayerScriptData handler = PlayerData.get(event.getEntityPlayer()).scriptData;
				if (handler.hadInteract) {
					...
					if (event.getItemStack().getItem() == CustomItems.scripted_item && !event.isCanceled()) {
						ItemScriptedWrapper isw = ItemScripted.GetWrapper(event.getItemStack());
						ItemEvent.InteractEvent eve = new ItemEvent.InteractEvent(isw, handler.getPlayer(), 0,
								(Object) null);
						event.setCanceled(EventHooks.onScriptItemInteract(isw, eve));
					}
	...
	public void invoke(LivingHurtEvent event) {
		if (event.getEntityLiving().world instanceof WorldServer) {
			...
			if (source instanceof EntityPlayer) {
				handler = PlayerData.get((EntityPlayer) source).scriptData;
				PlayerEvent.DamagedEntityEvent pevent = new PlayerEvent.DamagedEntityEvent(handler.getPlayer(),
	...
	public ScriptPlayerEventHandler registerForgeEvents() {
		...
		try {
			...
			List list = new ArrayList(ClassPath.from(this.getClass().getClassLoader())
					.getTopLevelClassesRecursive("net.minecraftforge.event"));
			list.addAll(ClassPath.from(this.getClass().getClassLoader())
					.getTopLevelClassesRecursive("net.minecraftforge.fml.common"));
			...
			while (true) {
							...
							try {
								...
								while (var17.hasNext()) {
									...
									if (c.getName().startsWith("com.pixelmonmod.pixelmon.api.events")
											&& Event.class.isAssignableFrom(c) && !Modifier.isAbstract(c.getModifiers())
				...
				while (var10.hasNext()) {
					...
					if (!GenericEvent.class.isAssignableFrom(c) && !EntityConstructing.class.isAssignableFrom(c)
							&& !PotentialSpawns.class.isAssignableFrom(c) && !RenderTickEvent.class.isAssignableFrom(c)
							&& !ClientTickEvent.class.isAssignableFrom(c)
							&& !GetCollisionBoxesEvent.class.isAssignableFrom(c)
							&& !ClientCustomPacketEvent.class.isAssignableFrom(c)
							&& !ItemTooltipEvent.class.isAssignableFrom(c) && Event.class.isAssignableFrom(c)
