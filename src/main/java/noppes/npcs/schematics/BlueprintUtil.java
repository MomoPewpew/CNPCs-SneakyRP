...
public class BlueprintUtil {
	...
	public static Blueprint createBlueprint(World world, BlockPos pos, short sizeX, short sizeY, short sizeZ,
		...
		Blueprint schem = new Blueprint(sizeX, sizeY, sizeZ, (short) ((byte) pallete.size()), states, structure, tes,
				requiredMods);
		if (name != null) {
			schem.setName(name);
		}
			...
			for (int i = 0; i < modListSize; ++i) {
				...
				if (!Loader.isModLoaded((String) requiredMods.get(i))) {
					Logger.getGlobal().log(Level.WARNING,
							"Couldn't load Blueprint, the following mod is missing: " + (String) requiredMods.get(i));
					return null;
				}
			}
