...
public class ModelNpcDragon extends ModelBase {
	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
		...
		for (int i = 0; i < 5; ++i) {
			...
			f9 = (float) ((double) f9
					- Math.cos((double) this.neck.rotateAngleY) * Math.cos((double) this.neck.rotateAngleX) * 10.0D);
			f10 = (float) ((double) f10
					- Math.sin((double) this.neck.rotateAngleY) * Math.cos((double) this.neck.rotateAngleX) * 10.0D);
			this.neck.render(f5);
		}
		...
		if (entitydragon.onGround) {
			for (k = 0; k < 2; ++k) {
				...
				this.frontLeg.rotateAngleX = MathHelper
						.cos((float) ((double) (f * 0.6662F) + (k == 0 ? 0.0D : 3.141592653589793D))) * 0.6F * f1
						+ 0.45F + f7 * 0.5F;
				this.frontLegTip.rotateAngleX = -1.3F - f7 * 1.2F;
				this.frontFoot.rotateAngleX = 0.85F + f7 * 0.5F;
				this.frontLeg.render(f5);
				this.rearLeg.rotateAngleX = MathHelper
						.cos((float) ((double) (f * 0.6662F) + (k == 0 ? 3.141592653589793D : 0.0D))) * 0.6F * f1
						+ 0.75F + f7 * 0.5F;
