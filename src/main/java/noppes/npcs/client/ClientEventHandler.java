			...
			if (te != null && te instanceof TileBuilder) {
				...
				if (schem != null) {
					...
					GlStateManager.translate(
							(double) TileBuilder.DrawPos.getX() - TileEntityRendererDispatcher.staticPlayerX,
							(double) TileBuilder.DrawPos.getY() - TileEntityRendererDispatcher.staticPlayerY + 0.01D,
							(double) TileBuilder.DrawPos.getZ() - TileEntityRendererDispatcher.staticPlayerZ);
					GlStateManager.translate(1.0F, (float) tile.yOffest, 1.0F);
					if (tile.rotation % 2 == 0) {
						this.drawSelectionBox(new BlockPos(schem.schema.getWidth(), schem.schema.getHeight(),
								schem.schema.getLength()));
					} else {
						this.drawSelectionBox(new BlockPos(schem.schema.getLength(), schem.schema.getHeight(),
								schem.schema.getWidth()));
					}
						...
						try {
							for (int i = 0; i < schem.size && i < 25000; ++i) {
								...
								if (state.getRenderType() != EnumBlockRenderType.INVISIBLE) {
									...
									GlStateManager.translate((float) pos.getX(), (float) pos.getY(),
											(float) pos.getZ());
