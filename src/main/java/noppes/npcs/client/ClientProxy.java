...
public class ClientProxy extends CommonProxy {
	...
	public void postload() {
		...
		RenderingRegistry.registerEntityRenderingHandler(EntityNpcAlex.class,
				new RenderCustomNpc(new ModelPlayerAlt(0.0F, true)));
		RenderingRegistry.registerEntityRenderingHandler(EntityNpcClassicPlayer.class,
				new RenderCustomNpc(new ModelClassicPlayer(0.0F)));
	...
	public void spawnParticle(EntityLivingBase player, String string, Object... ob) {
		if (string.equals("Block")) {
			...
			for (int i = 0; i < 2; ++i) {
				EntityEnderFX fx = new EntityEnderFX(npc, (rand.nextDouble() - 0.5D) * (double) player.width,
						rand.nextDouble() * (double) player.height - height - 0.25D,
						(rand.nextDouble() - 0.5D) * (double) player.width, (rand.nextDouble() - 0.5D) * 2.0D,
						-rand.nextDouble(), (rand.nextDouble() - 0.5D) * 2.0D, particles);
		...
		if (xx * xx + yy * yy + zz * zz <= 256.0D) {
			...
			if (fx != null) {
				if (particle == EnumParticleTypes.FLAME) {
					...
					ObfuscationReflectionHelper.setPrivateValue(ParticleSmokeNormal.class, (ParticleSmokeNormal) fx,
							scale, 0);
				}
	...
	public static class FontContainer {
		...
		public FontContainer(String fontType, int fontSize) {
			...
			try {
				if (!this.useCustomFont || fontType.isEmpty() || fontType.equalsIgnoreCase("default")) {
					this.textFont = new TrueTypeFont(new ResourceLocation("customnpcs", "opensans.ttf"), fontSize,
							1.0F);
				}
		...
		public int width(String text) {
			return this.useCustomFont ? this.textFont.width(text)
