...
public class EntityEnderFX extends ParticlePortal {
	...
	public void renderParticle(BufferBuilder renderer, Entity entity, float partialTicks, float par3, float par4,
		...
		renderer.pos((double) (f5 - par3 * f4 - par6 * f4), (double) (f6 - par4 * f4),
				(double) (f7 - par5 * f4 - par7 * f4)).tex((double) f1, (double) f3)
				.color(this.particleRed, this.particleGreen, this.particleBlue, 1.0F).lightmap(j, k).endVertex();
		renderer.pos((double) (f5 - par3 * f4 + par6 * f4), (double) (f6 + par4 * f4),
				(double) (f7 - par5 * f4 + par7 * f4)).tex((double) f1, (double) f2)
				.color(this.particleRed, this.particleGreen, this.particleBlue, 1.0F).lightmap(j, k).endVertex();
		renderer.pos((double) (f5 + par3 * f4 + par6 * f4), (double) (f6 + par4 * f4),
				(double) (f7 + par5 * f4 + par7 * f4)).tex((double) f, (double) f2)
				.color(this.particleRed, this.particleGreen, this.particleBlue, 1.0F).lightmap(j, k).endVertex();
		renderer.pos((double) (f5 + par3 * f4 - par6 * f4), (double) (f6 - par4 * f4),
				(double) (f7 + par5 * f4 - par7 * f4)).tex((double) f, (double) f3)
				.color(this.particleRed, this.particleGreen, this.particleBlue, 1.0F).lightmap(j, k).endVertex();
		tessellator.draw();
