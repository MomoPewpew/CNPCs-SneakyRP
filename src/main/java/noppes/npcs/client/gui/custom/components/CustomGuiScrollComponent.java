...
public class CustomGuiScrollComponent extends GuiCustomScroll implements IDataHolder, IClickListener {
	...
	public void onRender(Minecraft mc, int mouseX, int mouseY, int mouseWheel, float partialTicks) {
		...
		boolean hovered = mouseX >= this.guiLeft && mouseY >= this.guiTop && mouseX < this.guiLeft + this.getXSize()
				&& mouseY < this.guiTop + this.getYSize();
	...
	public ICustomGuiComponent toComponent() {
		CustomGuiScrollWrapper component = new CustomGuiScrollWrapper(this.id, this.guiLeft - GuiCustom.guiLeft,
				this.guiTop - GuiCustom.guiTop, this.width, this.height,
				(String[]) this.getList().toArray(new String[0]));
