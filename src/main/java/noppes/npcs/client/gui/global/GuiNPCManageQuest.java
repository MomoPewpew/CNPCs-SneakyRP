...
public class GuiNPCManageQuest extends GuiNPCInterface2
	...
	public void buttonEvent(GuiButton guibutton) {
		...
		if (button.id == 12) {
			guiyesno = new GuiYesNo(this, this.selectedQuest.title, I18n.translateToLocal("gui.deleteMessage"), 12);
			this.displayGuiScreen(guiyesno);
		}
	...
	public void subGuiClosed(SubGuiInterface subgui) {
		if (!(subgui instanceof SubGuiEditText) || !((SubGuiEditText) subgui).cancelled) {
			if (subgui.id == 1) {
				...
				for (category.title = ((SubGuiEditText) subgui).text; QuestController.instance
						.containsCategoryName(category); category.title = category.title + "_") {
				}
			...
			if (subgui.id == 3) {
				...
				for (this.selectedCategory.title = ((SubGuiEditText) subgui).text; QuestController.instance
						.containsCategoryName(this.selectedCategory); var10002.title = var10000.append(var10002.title)
								.append("_").toString()) {
				...
				Client.sendData(EnumPacketServer.QuestCategorySave,
						this.selectedCategory.writeNBT(new NBTTagCompound()));
			}

			if (subgui.id == 11) {
				Quest quest = new Quest(this.selectedCategory);

				for (quest.title = ((SubGuiEditText) subgui).text; QuestController.instance
						.containsQuestName(this.selectedCategory, quest); quest.title = quest.title + "_") {
				}

				Client.sendData(EnumPacketServer.QuestSave, this.selectedCategory.id,
