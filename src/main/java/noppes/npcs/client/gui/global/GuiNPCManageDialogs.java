	...
	public void subGuiClosed(SubGuiInterface subgui) {
		if (!(subgui instanceof SubGuiEditText) || !((SubGuiEditText) subgui).cancelled) {
			if (subgui.id == 1) {
				...
				for (category.title = ((SubGuiEditText) subgui).text; DialogController.instance
			...
			if (subgui.id == 3) {
				...
				for (this.selectedCategory.title = ((SubGuiEditText) subgui).text; DialogController.instance
				...
				Client.sendData(EnumPacketServer.DialogCategorySave,
			...
			if (subgui.id == 11) {
				...
				for (dialog.title = ((SubGuiEditText) subgui).text; DialogController.instance
						.containsDialogName(this.selectedCategory, dialog); dialog.title = dialog.title + "_") {
				}

				Client.sendData(EnumPacketServer.DialogSave, this.selectedCategory.id,
