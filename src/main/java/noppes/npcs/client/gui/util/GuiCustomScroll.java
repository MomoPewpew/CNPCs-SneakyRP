...
public class GuiCustomScroll extends GuiScreen {
	...
	public void setSize(int x, int y) {
		...
		if (this.listHeight > 0) {
			this.scrollHeight = (int) ((double) (this.ySize - 8) / (double) this.listHeight
					* (double) (this.ySize - 8));
		} else {
	...
	public void drawScreen(int i, int j, float f, int mouseScrolled) {
		if (this.visible) {
			this.drawGradientRect(this.guiLeft, this.guiTop, this.xSize + this.guiLeft, this.ySize + this.guiTop,
	...
	protected void drawItems() {
		for (int i = 0; i < this.list.size(); ++i) {
			...
			if (k >= 4 && k + 12 < this.ySize) {
				...
				if (this.multipleSelection && this.selectedList.contains(text)
