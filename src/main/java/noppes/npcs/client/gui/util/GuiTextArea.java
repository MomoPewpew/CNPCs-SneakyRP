	...
	public void drawScreen(int xMouse, int yMouse) {
		if (this.visible) {
			...
			if (this.clickScrolling) {
				...
				this.scrolledLine = Math.min(Math
						.max((int) (1.0F * (float) startBracket * (float) (yMouse - this.y) / (float) this.height), 0),
						startBracket);
			}
			...
			for (i = 0; i < list.size(); ++i) {
				...
				if (startBracket != endBracket) {
					if (startBracket >= data.start && startBracket < data.end) {
						...
						drawRect(this.x + 1 + yPos, e, this.x + 1 + posX, e + this.container.lineHeight + 1,
								-1728001024);
					}
					...
					if (wordHightLight != null) {
						...
						while (m.find()) {
							if (line.substring(m.start(), m.end()).equals(wordHightLight)) {
								...
								drawRect(this.x + 1 + posX, posY, this.x + 1 + e, posY + this.container.lineHeight + 1,
										-1728033792);
							}
						}
					}

					if (this.startSelection != this.endSelection && this.endSelection > data.start
			...
			if (this.hasVerticalScrollbar()) {
				...
				i = Math.max((int) (1.0F * (float) this.container.visibleLines / (float) this.container.linesCount
						* (float) this.height), 2);
				int posX = this.x + this.width - 6;
				int posY = (int) ((float) this.y + 1.0F * (float) this.scrolledLine / (float) this.container.linesCount
						* (float) (this.height - 4)) + 1;
				...
				if (i == 203) {
					...
					if (i == 200) {
						...
						if (i == 211) {
							...
							if (this.startSelection != this.endSelection) {
								NoppesStringUtils.setClipboardContents(
										this.text.substring(this.startSelection, this.endSelection));
								s = this.getSelectionBeforeText();
								this.setText(s + this.getSelectionAfterText());
	...
	private int cursorUp() {
		for (int i = 0; i < this.container.lines.size(); ++i) {
			...
			if (this.cursorPosition >= data.start && this.cursorPosition < data.end) {
				...
				return this.getSelectionPos(
						this.x + 1 + font.width(data.text.substring(0, this.cursorPosition - data.start)),
