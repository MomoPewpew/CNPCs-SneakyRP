...
public class GuiDialogInteract extends GuiNPCInterface implements IGuiClose {
	...
	private void drawWheel() {
		...
		while (true) {
				...
				do {
					do {
						if (!var3.hasNext()) {
							...
							this.drawTexturedModalRect(this.width / 2 + this.selectedX / 4 - 2,
									yoffset + 16 - this.selectedY / 6, 0, 0, 8, 8);
							return;
						}
			...
			if (slot == 4) {
				this.drawString(this.fontRenderer, option.title,
						this.width / 2 - 33 - ClientProxy.Font.width(option.title), yoffset - height / 2 + 14, color);
			}
	...
	private void drawLinedOptions(int j) {
		this.drawHorizontalLine(this.guiLeft - 60, this.guiLeft + this.xSize + 120,
				this.guiTop + this.dialogHeight - ClientProxy.Font.height((String) null) / 3, -1);
	...
	private void drawString(String text, int left, int color, int count) {
		...
		this.drawString(this.fontRenderer, text, this.guiLeft + left,
				this.guiTop + height * ClientProxy.Font.height((String) null), color);
	}
	...
	private void handleDialogSelection() {
		...
		if (this.dialog != null && this.dialog.hasOtherOptions() && !this.options.isEmpty()) {
			...
			if (option != null && option.optionType == 1) {
				this.lines.add(new TextBlockClient(this.player.getDisplayNameString(), option.title, 280,
						option.optionColor, new Object[] { this.player, this.npc }));
				this.calculateRowHeight();
