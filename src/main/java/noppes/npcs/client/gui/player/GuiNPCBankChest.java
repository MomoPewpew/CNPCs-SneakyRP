...
public class GuiNPCBankChest extends GuiContainerNPCInterface implements IGuiData {
		...
		if (this.maxSlots > 1) {
			for (int i = 0; i < this.maxSlots; ++i) {
				GuiNpcButton button = new GuiNpcButton(i, this.guiLeft - 50, this.guiTop + 10 + i * 24, 50, 20,
						I18n.translateToLocal("gui.tab") + " " + (i + 1));
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		if (!this.container.isAvailable()) {
			...
			ii = this.guiLeft + 30;
			y = this.guiTop + 8;
			this.fontRenderer.drawString(I18n.translateToLocal("bank.unlockCosts") + ":", ii, y + 4,
			...
			ii = this.guiLeft + 30;
			y = this.guiTop + 8;
			this.fontRenderer.drawString(I18n.translateToLocal("bank.upgradeCosts") + ":", ii, y + 4,
					CustomNpcResourceListener.DefaultTextColor);
			this.drawItem(ii + 90, y, this.currency, i, j);
		} else {
		...
		if (this.maxSlots > 1) {
			for (ii = 0; ii < this.maxSlots && this.availableSlots != ii; ++ii) {
				this.fontRenderer.drawString("Tab " + (ii + 1), this.guiLeft - 40, this.guiTop + 16 + ii * 24,
						16777215);
			}
		}
