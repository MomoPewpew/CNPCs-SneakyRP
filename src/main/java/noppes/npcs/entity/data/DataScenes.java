			...
			if (event.type == DataScenes.SceneType.MOVE) {
				...
				while (args.length > 1) {
					...
					if (args[0].startsWith("@")) {
						EntityLivingBase entitylivingbase = (EntityLivingBase) CommandBase.getEntity(
								DataScenes.this.npc.getServer(), DataScenes.this.npc, args[0], EntityLivingBase.class);
					...
					if (pos != null) {
						...
						if (move) {
							...
							DataScenes.this.npc.setPosition((double) pos.getX() + 0.5D, (double) pos.getY(),
				...
				if (event.type == DataScenes.SceneType.ROTATE) {
					...
					if (event.param.startsWith("@")) {
						entity = (EntityLivingBase) CommandBase.getEntity(DataScenes.this.npc.getServer(),
								DataScenes.this.npc, event.param, EntityLivingBase.class);
						DataScenes.this.npc.lookAi
								.rotate(DataScenes.this.npc.world.getClosestPlayerToEntity(entity, 30.0D));
					} else {
					...
					args = event.param.split(" ");
					EntityLivingBase entityx = (EntityLivingBase) CommandBase.getEntity(DataScenes.this.npc.getServer(),
							DataScenes.this.npc, args[0], EntityLivingBase.class);
					if (entityx == null) {
						return;
					}
					...
					NoppesUtilServer.runCommand(DataScenes.this.npc, DataScenes.this.npc.getName(), event.param,
							(EntityPlayer) null);
					...
					try {
						if (type.equals("walking_speed")) {
							DataScenes.this.npc.ais
									.setWalkingSpeed(ValueUtil.CorrectInt(Integer.parseInt(value), 0, 10));
