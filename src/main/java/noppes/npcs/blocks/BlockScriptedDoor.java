...
public class BlockScriptedDoor extends BlockNpcDoorInterface implements IPermission {
		...
		if (world.isRemote) {
			...
			if (iblockstate1.getBlock() != this) {
				...
				if (currentItem != null
					...
					NoppesUtilServer.sendOpenGui(player, EnumGuiType.ScriptDoor, (EntityNPCInterface) null,
					...
					if (EventHooks.onScriptBlockInteract(tile, player, side.getIndex(), hitX, hitY, hitZ)) {
						...
						this.toggleDoor(world, blockpos1,
								((Boolean) iblockstate1.getValue(BlockDoor.OPEN)).equals(false));
						return true;
					}
				}
			}
		...
		if (state.getValue(HALF) == EnumDoorHalf.UPPER) {
			...
			if (iblockstate2.getBlock() != this) {
				...
				if ((flag || neighborBlock.getDefaultState().canProvidePower()) && neighborBlock != this
						&& flag != (Boolean) iblockstate2.getValue(POWERED)) {
	...
	public void onBlockHarvested(World world, BlockPos pos, IBlockState state, EntityPlayer player) {
		BlockPos blockpos1 = state.getValue(HALF) == EnumDoorHalf.LOWER ? pos : pos.down();
		IBlockState iblockstate1 = pos.equals(blockpos1) ? state : world.getBlockState(blockpos1);
		if (player.capabilities.isCreativeMode && iblockstate1.getValue(HALF) == EnumDoorHalf.LOWER
