...
public class TileBorder extends TileNpcEntity implements Predicate, ITickable {
	...
	public void update() {
		if (!this.world.isRemote) {
			AxisAlignedBB box = new AxisAlignedBB((double) this.pos.getX(), (double) this.pos.getY(),
					(double) this.pos.getZ(), (double) (this.pos.getX() + 1),
					(double) (this.pos.getY() + this.height + 1), (double) (this.pos.getZ() + 1));
				...
				while (var3.hasNext()) {
					...
					if (entity instanceof EntityEnderPearl) {
						...
						if (!this.availability.isAvailable(player)) {
							...
							player.setPositionAndUpdate((double) pos2.getX() + 0.5D, (double) pos2.getY(),
									(double) pos2.getZ() + 0.5D);
							if (!this.message.isEmpty()) {
								player.sendStatusMessage(new TextComponentTranslation(this.message, new Object[0]),
										true);
							}
						}
					}
