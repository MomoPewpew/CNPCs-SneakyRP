...
public class BlockNpcRedstone extends BlockInterface implements IPermission {
	...
	public boolean onBlockActivated(World par1World, BlockPos pos, IBlockState state, EntityPlayer player,
		...
		if (par1World.isRemote) {
			...
			if (currentItem != null && currentItem.getItem() == CustomItems.wand
				...
				NoppesUtilServer.sendOpenGui(player, EnumGuiType.RedstoneBlock, (EntityNPCInterface) null, pos.getX(),
						pos.getY(), pos.getZ());
				return true;
			} else {
				return false;
	...
	public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase entityliving,
		...
		if (entityliving instanceof EntityPlayer && !world.isRemote) {
			NoppesUtilServer.sendOpenGui((EntityPlayer) entityliving, EnumGuiType.RedstoneBlock,
					(EntityNPCInterface) null, pos.getX(), pos.getY(), pos.getZ());
		}
