...
public class TrueTypeFont {
		...
		while (var10.hasNext()) {
			...
			if (gl.type != TrueTypeFont.GlyphType.NORMAL) {
				if (gl.type == TrueTypeFont.GlyphType.RESET) {
					...
					GlStateManager.color((float) (gl.color >> 16 & 255) / 255.0F,
							(float) (gl.color >> 8 & 255) / 255.0F, (float) (gl.color & 255) / 255.0F, 1.0F);
				}
			} else {
				GlStateManager.bindTexture(gl.texture);
				this.drawTexturedModalRect(i, 0.0F, (float) gl.x * this.textureScale(),
						(float) gl.y * this.textureScale(), (float) gl.width * this.textureScale(),
						(float) gl.height * this.textureScale());
	...
	public void drawTexturedModalRect(float x, float y, float textureX, float textureY, float width, float height) {
		...
		tessellator.noColor();
		tessellator.pos((double) x, (double) (y + height), (double) zLevel)
				.tex((double) (textureX * f), (double) ((textureY + height) * f1)).endVertex();
		tessellator.pos((double) (x + width), (double) (y + height), (double) zLevel)
				.tex((double) ((textureX + width) * f), (double) ((textureY + height) * f1)).endVertex();
		tessellator.pos((double) (x + width), (double) y, (double) zLevel)
				.tex((double) ((textureX + width) * f), (double) (textureY * f1)).endVertex();
		tessellator.pos((double) x, (double) y, (double) zLevel).tex((double) (textureX * f), (double) (textureY * f1))
				.endVertex();
