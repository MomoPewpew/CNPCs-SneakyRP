...
public class EntityWrapper implements IEntity {
	...
	public IRayTrace rayTraceBlock(double distance, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox) {
		...
		return result == null ? null
				: new RayTraceWrapper(NpcAPI.Instance().getIBlock(this.entity.world, result.getBlockPos()),
	...
	private IEntity[] findEntityOnPath(double distance, Vec3d vec3d, Vec3d vec3d1) {
		List list = this.entity.world.getEntitiesWithinAABBExcludingEntity(this.entity,
	...
	public void knockback(int power, float direction) {
		...
		this.entity.addVelocity((double) (-MathHelper.sin(v) * (float) power), 0.1D + (double) ((float) power * 0.04F),
				(double) (MathHelper.cos(v) * (float) power));
	...
	public void setPos(IPos pos) {
		this.entity.setPosition((double) ((float) pos.getX() + 0.5F), (double) pos.getY(),
