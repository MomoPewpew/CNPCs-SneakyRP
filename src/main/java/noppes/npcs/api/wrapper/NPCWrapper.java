...
public class NPCWrapper extends EntityLivingWrapper implements ICustomNpc {
	...
	public IProjectile shootItem(IEntityLivingBase target, IItemStack item, int accuracy) {
		if (item == null) {
			...
			return (IProjectile) NpcAPI.Instance().getIEntity(((EntityNPCInterface) this.entity)
					.shoot(target.getMCEntity(), accuracy, item.getMCItemStack(), false));
		}
	}
	...
	public IProjectile shootItem(double x, double y, double z, IItemStack item, int accuracy) {
		if (item == null) {
			...
			accuracy = ValueUtil.CorrectInt(accuracy, 1, 100);
			return (IProjectile) NpcAPI.Instance().getIEntity(
					((EntityNPCInterface) this.entity).shoot(x, y, z, accuracy, item.getMCItemStack(), false));
		}
	}
	...
	public String executeCommand(String command) {
		if (!((EntityNPCInterface) this.entity).getServer().isCommandBlockEnabled()) {
			...
			return NoppesUtilServer.runCommand(this.entity, ((EntityNPCInterface) this.entity).getName(), command,
					(EntityPlayer) null);
		}
	}
