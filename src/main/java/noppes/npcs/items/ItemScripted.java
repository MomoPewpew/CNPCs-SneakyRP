...
public class ItemScripted extends Item implements IPermission {
	...
	public boolean showDurabilityBar(ItemStack stack) {
		...
		return istack instanceof ItemScriptedWrapper ? ((ItemScriptedWrapper) istack).durabilityShow
				: super.showDurabilityBar(stack);
	}

	public double getDurabilityForDisplay(ItemStack stack) {
		IItemStack istack = NpcAPI.Instance().getIItemStack(stack);
		return istack instanceof ItemScriptedWrapper ? 1.0D - ((ItemScriptedWrapper) istack).durabilityValue
	...
	public int getRGBDurabilityForDisplay(ItemStack stack) {
		...
		if (!(istack instanceof ItemScriptedWrapper)) {
			...
			return color >= 0 ? color
					: MathHelper.hsvToRGB(Math.max(0.0F, (float) (1.0D - this.getDurabilityForDisplay(stack))) / 3.0F,
							1.0F, 1.0F);
		}
	}
	...
	public int getItemStackLimit(ItemStack stack) {
		IItemStack istack = NpcAPI.Instance().getIItemStack(stack);
		return istack instanceof ItemScriptedWrapper ? ((ItemScriptedWrapper) istack).getMaxStackSize()
