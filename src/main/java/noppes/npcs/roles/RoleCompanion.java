...
public class RoleCompanion extends RoleInterface {
	...
	public void aiUpdateTask() {
		...
		if (this.canAge && this.stage != EnumCompanionStage.FULLGROWN) {
			...
			} else if (this.stage == EnumCompanionStage.CHILD
					&& this.ticksActive > (long) EnumCompanionStage.TEEN.matureAge) {
				this.matureTo(EnumCompanionStage.TEEN);
			} else if (this.stage == EnumCompanionStage.TEEN
					&& this.ticksActive > (long) EnumCompanionStage.ADULT.matureAge) {
				this.matureTo(EnumCompanionStage.ADULT);
			} else if (this.stage == EnumCompanionStage.ADULT
					&& this.ticksActive > (long) EnumCompanionStage.FULLGROWN.matureAge) {
	...
	private void doEating() {
		if (this.eating != null && !this.eating.isEmpty()) {
			...
			if (this.npc.world.isRemote) {
				...
				for (int j = 0; j < 2; ++j) {
					Vec3d vec3 = new Vec3d(((double) rand.nextFloat() - 0.5D) * 0.1D, Math.random() * 0.1D + 0.1D,
							0.0D);
					vec3.rotateYaw(-this.npc.rotationPitch * 3.1415927F / 180.0F);
					vec3.rotatePitch(-this.npc.renderYawOffset * 3.1415927F / 180.0F);
					Vec3d vec31 = new Vec3d(((double) rand.nextFloat() - 0.5D) * 0.3D,
					...
					if (eating.getHasSubtypes()) {
						...
						this.npc.world.spawnParticle(EnumParticleTypes.ITEM_CRACK, vec31.x, vec31.y, vec31.z, vec3.x,
								vec3.y + 0.05D, vec3.z, new int[] { Item.getIdFromItem(eating.getItem()) });
					}
				}
			} else {
				--this.eatingTicks;
				if (this.eatingTicks <= 0) {
					...
					rand = this.npc.getRNG();
					this.npc.playSound(SoundEvents.ENTITY_GENERIC_EAT, 0.5F + 0.5F * (float) rand.nextInt(2),
	...
	private boolean isWeapon(ItemStack item) {
		if (item != null && item.getItem() != null) {
			return item.getItem() instanceof ItemSword || item.getItem() instanceof ItemBow
	...
	public boolean canWearArmor(ItemStack item) {
		...
		if (item != null && item.getItem() instanceof ItemArmor && level > 0) {
			if (level >= 5) {
				...
				int reduction = (Integer) ObfuscationReflectionHelper.getPrivateValue(ArmorMaterial.class,
	...
	public void setSitting(boolean sit) {
		if (sit) {
			...
			this.npc.setPositionAndUpdate((double) this.npc.getStartXPos(), this.npc.posY,
	...
	public boolean hasInv() {
		if (!this.hasInv) {
			...
			return this.hasTalent(EnumCompanionTalent.INVENTORY) || this.hasTalent(EnumCompanionTalent.ARMOR)
					|| this.hasTalent(EnumCompanionTalent.SWORD);
		}
	}
