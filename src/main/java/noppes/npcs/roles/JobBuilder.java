		...
		if ((!this.build.finished || this.placingList != null) && this.build.enabled && !this.build.isInvalid()) {
			if (this.ticks++ >= 10) {
				...
				if ((this.placingList == null || this.placingList.isEmpty()) && this.placing == null) {
					...
					this.npc.getNavigator().tryMoveToXYZ((double) this.placing.pos.getX(),
							(double) (this.placing.pos.getY() + 1), (double) this.placing.pos.getZ(), 1.0D);
					if (this.tryTicks++ > 40 || this.npc.nearPosition(this.placing.pos)) {
						...
						if (this.tryTicks > 40) {
							...
							this.npc.setPositionAndUpdate((double) blockPos.getX() + 0.5D, (double) blockPos.getY(),
			...
			this.build = null;
			this.npc.getNavigator().tryMoveToXYZ((double) this.npc.getStartXPos(), this.npc.getStartYPos(),
					(double) this.npc.getStartZPos(), 1.0D);
		}
	}

	private String blockToString(BlockData data) {
		return data.state.getBlock() == Blocks.AIR ? Items.IRON_PICKAXE.getRegistryName().toString()
				: this.itemToString(data.getStack());
	}
