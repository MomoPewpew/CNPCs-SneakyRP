...
public class JobSpawner extends JobInterface implements IJobSpawner {
	...
	public boolean shouldDelete(EntityLivingBase entity) {
		return !this.npc.isInRange(entity, 60.0D) || entity.isDead || entity.getHealth() <= 0.0F
				|| this.despawnOnTargetLost && this.target == null;
	}
	...
	public void aiStartExecuting() {
		...
		for (Iterator var1 = this.spawned.iterator(); var1.hasNext(); this.setTarget(entity,
				this.npc.getAttackTarget())) {
	...
	private List getNearbySpawned() {
		...
		List list = this.npc.world.getEntitiesWithinAABB(EntityLivingBase.class,
