...
public class JobFarmer extends JobInterface implements MassBlockController.IMassBlock, IJobFarmer {
	...
	private void chest() {
		...
		this.npc.getLookHelper().setLookPosition((double) pos.getX(), (double) pos.getY(), (double) pos.getZ(), 10.0F,
	...
	private void pluck() {
		...
		if (this.npc.nearPosition(pos) || this.walkTicks++ > 400) {
			if (this.walkTicks > 400) {
				...
				this.npc.setPositionAndUpdate((double) pos.getX() + 0.5D, (double) pos.getY(),
	...
	public void aiUpdateTask() {
		...
		if (this.ripe != null) {
			this.npc.getNavigator().clearPath();
			this.npc.getLookHelper().setLookPosition((double) this.ripe.getX(), (double) this.ripe.getY(),
					(double) this.ripe.getZ(), 10.0F, (float) this.npc.getVerticalFaceSpeed());
		}
