...
public class RoleTransporter extends RoleInterface implements IRoleTransporter {
			...
			if (!this.hasTransport()) {
				...
				if (loc.type != 0) {
					...
					List inRange = this.npc.world.getEntitiesWithinAABB(EntityPlayer.class,
							this.npc.getEntityBoundingBox().grow(6.0D, 6.0D, 6.0D));
	...
	public void transport(EntityPlayerMP player, String location) {
		...
		if (loc != null && (loc.isDefault() || playerdata.transports.contains(loc.id))) {
			...
			if (!EventHooks.onNPCRole(this.npc, event)) {
				NoppesUtilPlayer.teleportPlayer(player, (double) loc.pos.getX(), (double) loc.pos.getY(),
						(double) loc.pos.getZ(), loc.dimension);
			}
		}
	}
	...
	private void unlock(EntityPlayer player, TransportLocation loc) {
		...
		if (!data.transports.contains(this.transportId)) {
			RoleEvent.TransporterUnlockedEvent event = new RoleEvent.TransporterUnlockedEvent(player,
					this.npc.wrappedNPC);
			if (!EventHooks.onNPCRole(this.npc, event)) {
