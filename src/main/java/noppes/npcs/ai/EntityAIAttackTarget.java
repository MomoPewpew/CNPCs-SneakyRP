...
public class EntityAIAttackTarget extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if (entitylivingbase != null && entitylivingbase.isEntityAlive()) {
			...
			if (this.npc.inventory.getProjectile() != null
					&& (melee <= 0 || !this.npc.isInRange(entitylivingbase, (double) melee))) {
				return false;
			} else {
	...
	public boolean shouldContinueExecuting() {
		...
		if (this.entityTarget != null && this.entityTarget.isEntityAlive()) {
			if (!this.npc.isInRange(this.entityTarget, (double) this.npc.stats.aggroRange)) {
				...
				int melee = this.npc.stats.ranged.getMeleeRange();
				return melee > 0 && !this.npc.isInRange(this.entityTarget, (double) melee) ? false
	...
	public void updateTask() {
		...
		double range = (double) ((float) (this.npc.stats.melee.getRange() * this.npc.stats.melee.getRange())
				+ this.entityTarget.width);
