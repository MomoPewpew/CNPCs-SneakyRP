...
public class EntityAIWatchClosest extends EntityAIBase {
	...
	public boolean shouldExecute() {
		if (this.npc.getRNG().nextFloat() < this.chance && !this.npc.isInteracting()) {
			...
			if (this.watchedClass == EntityPlayer.class) {
				...
				this.closestEntity = this.npc.world.findNearestEntityWithinAABB(this.watchedClass, this.npc
						.getEntityBoundingBox().grow((double) this.maxDistance, 3.0D, (double) this.maxDistance),
						this.npc);
	...
	public boolean shouldContinueExecuting() {
		if (!this.npc.isInteracting() && !this.npc.isAttacking() && this.closestEntity.isEntityAlive()
