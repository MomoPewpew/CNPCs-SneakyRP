...
public class EntityAIReturn extends EntityAIBase {
	...
	public boolean shouldExecute() {
		if (!this.npc.hasOwner() && !this.npc.isRiding() && this.npc.ais.shouldReturnHome() && !this.npc.isKilled()
			...
			if (this.npc.ais.findShelter == 0 && (!this.npc.world.isDaytime() || this.npc.world.isRaining())
				...
				pos = new BlockPos((double) this.npc.getStartXPos(), this.npc.getStartYPos(),
			...
			if (this.npc.isAttacking()) {
				...
				return true;
			} else if (this.npc.ais.getMovingType() == 2 && this.npc.ais
					.getDistanceSqToPathPoint() < (double) (CustomNpcs.NpcNavRange * CustomNpcs.NpcNavRange)) {
				return false;
				...
				return !this.npc.isInRange((double) this.npc.getStartXPos(), -1.0D, (double) this.npc.getStartZPos(),
						(double) this.npc.ais.walkingRange);
	...
	public boolean shouldContinueExecuting() {
		if (!this.npc.isFollower() && !this.npc.isKilled() && !this.npc.isAttacking()
	...
	private void navigate(boolean towards) {
		...
		if (range > (double) CustomNpcs.NpcNavRange || towards) {
			...
			if (distance > 2) {
				...
				Vec3d pos = RandomPositionGenerator.findRandomTargetBlockTowards(this.npc, distance,
