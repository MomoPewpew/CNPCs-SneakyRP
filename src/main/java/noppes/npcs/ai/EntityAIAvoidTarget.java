...
public class EntityAIAvoidTarget extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if (target == null) {
			...
			if (this.targetEntityClass == EntityPlayer.class) {
				this.closestLivingEntity = this.npc.world.getClosestPlayerToEntity(this.npc,
						(double) this.distanceFromEntity);
				if (this.closestLivingEntity == null) {
					return false;
				}
			} else {
				List var1 = this.npc.world.getEntitiesWithinAABB(this.targetEntityClass, this.npc.getEntityBoundingBox()
						.grow((double) this.distanceFromEntity, 3.0D, (double) this.distanceFromEntity));
				if (var1.isEmpty()) {
					return false;
				}
			...
			if (!this.npc.getEntitySenses().canSee(this.closestLivingEntity) && this.npc.ais.directLOS) {
				...
				Vec3d var2 = RandomPositionGenerator.findRandomTargetBlockAwayFrom(this.npc, 16, 7, new Vec3d(
				...
				if (var2 == null) {
					return false;
				} else if (this.closestLivingEntity.getDistanceSq(var2.x, var2.y, var2.z) < this.closestLivingEntity
						.getDistanceSq(this.npc)) {
					return false;
	...
	public void updateTask() {
		...
		if (this.npc.ais.tacticalVariant == 3
				&& (!this.npc.isInRange(this.closestLivingEntity, (double) this.distanceFromEntity)
						|| this.npc.isInRange(this.closestLivingEntity, (double) this.npc.ais.getTacticalRange()))) {
			this.health = this.npc.getHealth();
		}
