...
public class EntityAIOrbitTarget extends EntityAIBase {
	...
	public boolean shouldExecute() {
		if (--this.delay > 0) {
			...
			if (this.targetEntity == null) {
				...
				return !this.npc.isInRange(this.targetEntity, (double) (this.distance / 2.0F))
						&& (this.npc.inventory.getProjectile() != null
								|| this.npc.isInRange(this.targetEntity, (double) this.distance));
			}
		}
	}

	public boolean shouldContinueExecuting() {
		return this.targetEntity.isEntityAlive()
				&& !this.npc.isInRange(this.targetEntity, (double) (this.distance / 2.0F))
				&& this.npc.isInRange(this.targetEntity, (double) this.distance * 1.5D) && !this.npc.isInWater()
				&& this.canNavigate;
	}
	...
	public void updateTask() {
		...
		if (this.npc.getNavigator().noPath() && this.tick >= 0 && this.npc.onGround && !this.npc.isInWater()) {
			...
			this.tick = MathHelper.ceil(this.npc.getDistance(this.movePosX, this.movePosY, this.movePosZ)
					/ (double) (this.npc.getSpeed() / 20.0F));
			if (this.decay) {
