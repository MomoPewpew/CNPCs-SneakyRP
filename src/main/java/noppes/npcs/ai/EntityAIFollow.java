...
public class EntityAIFollow extends EntityAIBase {
	...
	public void updateTask() {
		...
		if (this.updateTick >= 10) {
			this.updateTick = 0;
			this.npc.getLookHelper().setLookPositionWithEntity(this.owner, 10.0F,
			...
			if (!this.npc.getNavigator().tryMoveToEntityLiving(this.owner, speed)
					&& !this.npc.isInRange(this.owner, 16.0D)) {
